window.imager_tools = function(){
	...
	this.scrape = function(site){
		...
		function go(){
			...
			if(self.site==='wikipedia' || self.site === 'google'){
				return scrapeResult(self.ready());
			}
			var count = 0;
			self.scroll(wh,'y').then(function(){
				console.log('at the bottom');
				count++;
				if(count === 2) scrapeResult(self.ready());
			});
	...
	this.fix = function(href){
		...
		if(href.indexOf('data:image') === 0) return false;
	...
	this.ready = function(){
		...
		try{
			...
			if(this.site === 'google'){
				i$('script').each(function(){
					let data = i$(this).html();
					if(data.startsWith('AF_initDataCallback(')) {
						data = data.replace('AF_initDataCallback', 'googleScrape');
						googleScrape(data);						
					}
				});
				return {images:[],title:this.title,description:this.description,favicon:favicon};
			}
			...
			i$('div,a,span,body,li,main,td,table,tr,th,source').each(function(){
				self.search(i$(this));
			})
			return  {images:this.foo,title:this.title,description:this.description,favicon:favicon};
		}
		catch(err){
